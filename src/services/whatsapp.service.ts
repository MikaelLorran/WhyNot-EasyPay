...
export async function connectToWhatsApp(): Promise<WASocket> {
    ...
    if (sock && (sock.user as any)?.id && sock.ws.readyState === sock.ws.OPEN) {
        return sock;
    }
    ...
    const initiateConnection = async (): Promise<WASocket> => {
         ...
         sock.ev.on('connection.update', async (update: Partial<ConnectionState>) => {
            ...
            if (connection === 'close') {
                ...
                if (shouldReconnect) {
                    ...
                    console.log('Sessão encerrada (logged out). Exclua a pasta de sessão e tente novamente.');
                    sock = null;
                }
        ...
        sock.ev.on('messages.upsert', async (m) => {
            ...
            if (!msg.key.fromMe && m.type === 'notify') {
                ...
                const text = (msg.message?.extendedTextMessage?.text || msg.message?.conversation || '').toLowerCase();
                ...
                if (text) {
                    if (sock && sock.ws.readyState === sock.ws.OPEN) {
                        ...
                        console.warn('⚠️ Não foi possível enviar "pong!" - Conexão Baileys não está aberta (Socket fechado).');
                    }
                }
            }
        });
...
export async function sendMessage(number: string, content: AnyMessageContent) {
    ...
    if (!client || client.ws.readyState !== client.ws.OPEN) {
        console.log("Cliente não conectado ou desconectado. Tentando obter ou estabelecer conexão...");
        try {
    ...
    if (!client || client.ws.readyState !== client.ws.OPEN) {
        console.error("Não foi possível estabelecer a conexão para enviar a mensagem.");
        return;
    }
